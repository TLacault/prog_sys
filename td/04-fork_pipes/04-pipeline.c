...
#else
char *ls[] = { "ls", "-l", NULL };
// ".c" est une regex ("." = n'importe quel char). Pour matcher un vrai ".c" en fin de ligne:
char *grep[] = { "grep", "\\.c$", NULL };
char *wc[] = { "wc", "-l", NULL };
char **cmd[] = { ls, grep, wc };
#endif

#define NB_COMMAND ((int)(sizeof(cmd) / sizeof(cmd[0])))
...
int main(int ac, char **av) {
    (void)ac;
    (void)av;
    return run_pipeline(cmd, NB_COMMAND);
}
